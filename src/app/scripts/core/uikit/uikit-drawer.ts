...
export class Drawer {
  ...
  constructor(element, options) {
    ...
    if (!element) {
      throw new Error(
        'Missing required attribute: target element (css selector or DOM node) must be provided'
      )
      ...
      throw new Error(
        'Incorrect type: target element must be DOM node or string css selector'
      )
    }
    this.targetElement.classList.add(CLASS_NAME_DRAWER)
    if (this.config.embed) {
      this.targetElement.parentElement.classList.add(
        CLASS_NAME_DRAWER_EMBED_CONTAINER
      )
    }
    this.containerElement = this.targetElement.querySelector(
      SELECTOR_DRAWER_CONTAINER
    )
  ...
  __isStart(value) {
    if (this.__isBoolean(value)) {
      ...
      value === 'save' &&
      window.localStorage &&
      window.localStorage.getItem(this.config.nameOfStore) !== null
    ) {
      if (
        JSON.parse(window.localStorage.getItem(this.config.nameOfStore)) ===
        true
      ) {
        this.open()
      } else {
  ...
  __isDOMElement(target) {
    return (
      ...
      (target.nodeType === document.ELEMENT_NODE ||
        target.nodeType === document.DOCUMENT_FRAGMENT_NODE)
    )
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document
    .querySelectorAll('.js-drawer')
    .forEach((element) => Drawer.init(element, options))
})
