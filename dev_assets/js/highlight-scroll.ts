...
import { isTabletLayout } from '../../src/app/scripts/utils/layout'
import { startScrollTo } from './scroll-to'
import { onScroll } from '../../src/app/scripts/utils/scroll-observer'

export class HighlightScroll {
  container: any
  anchors: any
  busy: boolean
  ...
  addEvents() {
    ...
    document.addEventListener('endScroll', (e) => {
      const targetId = (<any>e).detail.targetId
  ...
  checkInView() {
    ...
    for (let i = 0; i < this.anchors.length; i++) {
      ...
      const targetElem = this.container.querySelector(
        `[data-id="${anchor.dataset.anchor}"]`
      )
  ...
  static init() {
    ...
    if (!container || 'init' in (<HTMLElement>container).dataset) return false
    container.setAttribute('data-init', String(true))
